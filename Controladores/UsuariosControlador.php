...
if (isset($_REQUEST["nU"])) {
    if (isset($_REQUEST["adm"])) {
        $sql =
            "UPDATE usuarios SET rol='Admin' WHERE nombreUsuario='" .
            $_REQUEST["nU"] .
            "'";
    ...
    if (isset($_REQUEST["user"]) and $_REQUEST["nU"] != "Rob") {
        $sql =
            "UPDATE usuarios SET rol='Usuario' WHERE nombreUsuario='" .
            $_REQUEST["nU"] .
            "'";
        $comprobar = $conexion->query($sql);
    } elseif ($_REQUEST["nU"] == "Rob") {
    }
...
for ($cont2 = 0; $cont < $num; $cont2++) {
    ...
    if ($mem && $mem->num_rows > 0) {
        ...
        $datosUsuarios[$info["nombreUsuario"]]["domicilio"] =
            $info["domicilio"];
        $info["fechaNacimiento"] = date(
            "d-m-Y",
            strtotime($info["fechaNacimiento"])
        );
        $datosUsuarios[$info["nombreUsuario"]]["fechaNacimiento"] =
            $info["fechaNacimiento"];
        $datosUsuarios[$info["nombreUsuario"]]["rol"] = $info["rol"];
        $datosUsuarios[$info["nombreUsuario"]]["apellidos"] =
            $info["apellidos"];
        $datosUsuarios[$info["nombreUsuario"]]["codigoPostal"] =
            $info["codigoPostal"];
        $datosUsuarios[$info["nombreUsuario"]]["correo"] = $info["correo"];
        $datosUsuarios[$info["nombreUsuario"]]["provincia"] =
            $info["provincia"];
        $datosUsuarios[$info["nombreUsuario"]]["población"] =
            $info["población"];
        $datosUsuarios[$info["nombreUsuario"]]["numeroMovil"] =
            $info["numeroMovil"];
