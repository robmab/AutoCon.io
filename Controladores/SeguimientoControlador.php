...
if (isset($_REQUEST["vehiculo"])) {
    ...
    $sql =
        "SELECT id FROM vehiculos WHERE modelo='" . $_REQUEST["vehiculo"] . "'";
    $memi = $conexion->query($sql);
    ...
    $sql =
        "SELECT * FROM vehiculos WHERE modelo='" . $_REQUEST["vehiculo"] . "'";
    $memi = $conexion->query($sql);
    if ($memi->num_rows > 0) {
        $info = $memi->fetch_array();
    ...
    if (isset($_REQUEST["R"])) {
        ...
        $_SESSION["mensajeBD1"] =
            "Alquiler cancelado con exito. Devuelve el vehículo en un plazo de 1 semana.";
    }
}
...
if (isset($_REQUEST["nombre"])) {
    ...
    $sql =
        "SELECT id FROM componentes WHERE nombre='" .
        $nombre .
        "' AND tipo='" .
        $tipo .
        "'";
    ...
    if (isset($_REQUEST["unico"])) {
        $sql =
            "DELETE FROM componente_usuario WHERE n='" . $_REQUEST["n"] . "' ";
        $comprobar = $conexion->query($sql);
        ...
        $sql =
            "SELECT * FROM componente_usuario WHERE n='" .
            $_REQUEST["n"] .
            "' ";
        $memi = $conexion->query($sql);
        ...
        $_SESSION["mensajeBD2"] =
            "1 " . $nombre . " de tipo " . $tipo . " cancelado.";
...
for ($cont2 = 0; $cont < $num; $cont2++) {
    ...
    if ($mem && $mem->num_rows > 0) {
        ...
        if ($info["usuario"] == $usuario) {
            ...
            $sql =
                "SELECT *  FROM vehiculos WHERE id='" . $info["vehiculo"] . "'";
