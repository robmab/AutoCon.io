...
if (isset($_REQUEST["email"])) {
    ...
    for ($cont2 = 0; $cont < $num; $cont2++) {
        $sql =
            "SELECT nombreUsuario,correo  FROM usuarios WHERE ID='" .
            $cont2 .
            "'";
    ...
    foreach ($listaUsu as $usu) {
        if ($usu == $email) {
            $sql =
                "SELECT * FROM usuarios WHERE nombreUsuario='" . $email . "'";
            $mem = $conexion->query($sql);
            if ($mem && $mem->num_rows > 0) {
                $info = $mem->fetch_array();
    ...
    $sql =
        "SELECT contraseña FROM usuarios WHERE nombreUsuario='" .
        $email .
        "' OR correo='" .
        $email .
        "'  ";
    ...
    $sql =
        "SELECT rol FROM usuarios WHERE nombreUsuario='" .
        $email .
        "' OR correo='" .
        $email .
        "'  ";
