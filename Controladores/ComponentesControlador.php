...
if (isset($_REQUEST['comprar'])) {
  ...
  while ($contad == 0){
   ...
   //Actualizar disponibilidad -->
    ...
    if( $memi->num_rows > 0 ){
        $info=$memi->fetch_array() ; 
        $cantidadC= $info['cantidad']   ;$cantidadC = $cantidadC -1   ;
    };
   ...
   //----------------------------
    ...
    $_SESSION['mensajeBD']="Componente ".  $nombre ." - ".  $tipo ." comprado con éxito.";    
    header("Location:ComponentesControlador.php");    
    exit;
...
if ( $mem&&$mem->num_rows>0 ) {
 ...
 $listaComponentes[$info['nombre']][$info['tipo']]['descuento']=  $info['descuento'] ;
      ...
      $listaComponentes[$info['nombre']][$info['tipo']]['precioO']=  $info['precio'] *1;
        ...
        $precioR = $info['descuento'] /100; $precioR = $precioR -1; $precioR = $precioR * -1 ;$precioR =$precioR * $info['precio'] ;
...
if ( $mem&&$mem->num_rows>0 ) {
    $info2= $mem->fetch_array() ;
     ...
     $porciento=$info2['porciento']; $porciento = $porciento /100 ; $porciento = $porciento -1 ;  $porciento = $porciento * -1 ;
